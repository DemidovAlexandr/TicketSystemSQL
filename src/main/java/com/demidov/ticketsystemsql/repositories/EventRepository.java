...
public interface EventRepository extends JpaRepository<Event, Integer> {
    ...
    Optional<List<Event>> findAllByArtistList(List<Artist> artistList);

    Optional<List<Event>> findAllByBeginDateTimeOrderByNameAsc(ZonedDateTime dateTime);

    @Query("select e from Event e where e.beginDateTime = ?1 and e.genre = ?2 and e.venue.city = ?3 order by e.name")
    Optional<List<Event>> findAllByDateAndGenreAndCity(ZonedDateTime dateTime, Genre genre, String cityName);
